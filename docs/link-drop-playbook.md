...
4. Create or update a note under `content/notes/` with:
   ...
   - tweet and YouTube links rendered with Zola/Tera components, `{{<tweet id="..." url="..."/>}}` or `{{<youtube id="..." url="..."/>}}`, when they appear in the body. X embeds must remain opt-in: render a local placeholder first, store consent in browser `localStorage`, and only load `platform.x.com/widgets.js` after the reader clicks to enable X embeds
   - `[extra].source_url`
   - `[extra].source_type`
